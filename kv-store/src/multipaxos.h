...
class MultiPaxos : public multipaxos::MultiPaxosRPC::Service {
 public:
  ...
  Result Replicate(multipaxos::RPC_Command command, int64_t client_id);

  void PrepareThread();
  void CommitThread();

  int64_t RunPreparePhase(int64_t ballot);
...
struct prepare_state_t {
  explicit prepare_state_t() : num_rpcs_(0), num_oks_(0), max_last_index_(0) {}
  size_t num_rpcs_;
  size_t num_oks_;
  int64_t max_last_index_;
  std::unordered_map<int64_t, multipaxos::RPC_Instance> log_;
  std::mutex mu_;
  std::condition_variable cv_;
};

struct accept_state_t {
  explicit accept_state_t() : num_rpcs_(0), num_oks_(0) {}
  size_t num_rpcs_;
  size_t num_oks_;
  std::mutex mu_;
  std::condition_variable cv_;
